...
def train_or_test(model, data_loader, optimizer, loss_op, device, args, epoch, mode='training'):
    ...
    for batch_idx, (data, labels) in enumerate(tqdm(data_loader)):  # Capture labels here
        data = data.to(device)
        labels = labels.to(device)  # Ensure labels are also moved to the correct device
        model_output = model(data, labels)  # Modify the model call if it requires labels
        loss = loss_op(data, model_output, labels)  # Pass labels to loss function if needed
